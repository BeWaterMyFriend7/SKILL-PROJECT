...
SPEC = importlib.util.spec_from_file_location("xml_palette", PATH)
...
class PaletteTests(unittest.TestCase):
    ...
    def test_svg_palette_matches_when_both_skills_are_installed(self):
        ...
        if not sibling.is_file():
            self.skipTest("svg-generator skill is not installed beside xml-diagram")
        spec = importlib.util.spec_from_file_location("svg_palette_peer", sibling)
        module = importlib.util.module_from_spec(spec)
        assert spec.loader
        spec.loader.exec_module(module)
        for theme in PALETTE.THEMES:
            for mode in ("light", "dark"):
                self.assertEqual(PALETTE.build_palette(theme, mode), module.build_palette(theme, mode))
