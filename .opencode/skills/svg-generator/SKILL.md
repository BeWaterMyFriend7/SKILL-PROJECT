---
name: svg-generator
description: 根据自然语言描述生成 SVG 代码。支持流程图、卡片、图表、图标等常用图形，输出可直接在浏览器中查看的 SVG 文件，支持 drawio 直接编辑。当用户提到 SVG、生成图片、图形、图标、流程图时触发。
---
...
生成的 SVG 支持 drawio 直接打开编辑：

### 兼容性设计原则

1. **标准 SVG 格式**：使用 W3C 标准 SVG 1.1 规范，避免使用 drawio 不支持的特性
2. **清晰的分组结构**：使用 `<g>` 元素合理分组，便于在 drawio 中批量选择和移动
3. **精确的坐标系统**：所有坐标使用绝对定位，避免相对定位导致的编辑错位
4. **完整的视图范围**：viewBox 必须覆盖所有可见元素，包括阴影和箭头
5. **充足的边距**：四周保留 ≥40px 边距，确保导入后不会自动裁剪
6. **简化的滤镜效果**：仅使用 feDropShadow 滤镜，避免复杂滤镜导致兼容性问题
7. **标准字体**：使用系统默认字体（PingFang SC, Microsoft YaHei, sans-serif），避免字体加载问题
...
<g id="流程组-1">
  <rect x="100" y="100" width="140" height="60" rx="8" 
        fill="#fff" stroke="#3498db" stroke-width="2"/>
  <text x="170" y="135" text-anchor="middle" font-size="14" fill="#333">节点名称</text>
</g>

<!-- 避免：过度嵌套和复杂变换 -->
<!-- 不使用：transform="translate(...)" 嵌套，会导致 drawio 编辑困难 -->
```

### 导入导出指南

**导入 drawio：**
1. 打开 drawio，选择「文件」→「导入」→「SVG」
2. 或直接拖拽 SVG 文件到画布
3. 导入后，使用「组合/取消组合」功能编辑单个元素

**从 drawio 导出：**
1. 编辑完成后，选择「文件」→「导出为」→「SVG」
2. 勾选「包含绘图边界」确保内容完整
3. 取消勾选「嵌入图像」，保持文件轻量
