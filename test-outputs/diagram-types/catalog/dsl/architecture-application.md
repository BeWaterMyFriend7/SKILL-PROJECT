# 应用架构 DSL：智能交易平台应用架构

生成方式：fresh-catalog-v1
规格哈希：0c6d2c6e96995aec4fcae9b659df09b73d51da2916de23c1f9a53a308e1974d5

## 原始自然语言需求

为架构评审绘制智能交易平台应用架构，纵向展示渠道、应用、服务、数据与基础设施，突出统一接入与交易编排，右侧补充治理保障。

## 可执行蓝图

- 图形类型：architecture-application（应用架构）
- 核心结论：统一接入连接业务应用与领域服务，并由治理能力保障稳定运行。
- 视觉骨架：分层架构
- 视觉主题：Tech Blue｜科技蓝（默认）
- 布局：vertical-stack
- 主叙事轴：y
- 视觉合同：enterprise-v2
- 附加模块：aux-column、focus-node、footer-band、top-band
- 图标策略：仅在入口、焦点或关键区域按内容少量使用自包含线性图标，不设置硬上限

## 结构颜色映射

- axis-main: #3B6EDC（来源：main-blue）
- axis-cross: #2AA7C8（来源：data-cyan）
- side-rail: #2D56B3（来源：deep-blue）
- focus: #3B6EDC（来源：main-blue）
- data-flow: #5B8FF9（来源：medium-blue）
- group: #5B8FF9（来源：medium-blue）
- connector: #3B6EDC（来源：main-blue）

## 质量预期

页面标题、区域、卡片和辅助信息层级清楚；唯一焦点承载核心结论；同类组件颜色一致；连线不穿越节点；XML/SVG 节点、角色和几何语义一致。
