...
- 重构 code-change-plan 与 code-merge-helper，使报告同时服务人工审批和 AI 执行。
- 保留原有四章、五章主结构，章内改为结论优先的执行契约。
- `agent-offline-mermory` 使用 `Tasks`、`Knowledge`、`Daily` 三类记录，并提供 Obsidian 可视化首页。

### 主要变化

- 代码变更方案新增范围外、当前实现证据、必须保持、文件级修改位置、停止条件和验收验证映射。
- 合并报告新增操作类型与 SHA、stage 实际来源、必须同时保留、语义冲突、逐文件暂存和双侧组合验证。
- 报告压缩不再依赖机械条数或字数限制，改为信息去重、表格化和按风险展开。
- 移除报告末尾的“方案结束”“报告结束”和装饰性分隔线，正文在最后一项有效内容后直接结束。
- 删除与现有模板冲突且不再需要的 validate_change_plan.py 和 validate_merge_plan.py。
- 更新两个 Skill 的使用文档与完整输出示例。
- Obsidian 首页提供任务数、知识数、每日总结数、搜索、四区滚动列表和索引快捷跳转。
- 新增任务、知识、总结三选一的手动记录弹窗，直接按对应模板创建 Markdown 文件。
- 每日总结模板使用“完成 / 问题 / 待办”结构；使用手册同步当前目录、模板和触发规则，并加入首页与弹窗截图。
