...
`xml-diagram` 用于根据自然语言需求生成可继续编辑的 Draw.io XML 图形文件，当前支持业务、应用、技术、部署、数据架构以及流程、状态、时序、关系和结构化总结类图形。
...
3. 匹配对应模板，生成 Draw.io XML。所有架构图统一采用“一级标题外置、一级容器独立、二级标题条居中、内容紧凑网格”的骨架；数据架构只增加候选层级和主数据流语义，不改变视觉规范。
...
数据架构图不会固定套用七层。生成前会根据需求对数据源、接入、传输缓冲、计算加工、存储模型、数据服务和数据应用等候选层进行保留、合并、删除、拆分或新增；不保留空层。每一层仍按标准架构图拆为二级分组卡片，数据治理、安全和运维也使用同款架构区域表达。数据架构图表达平台分层、数据流向和能力边界，ER 图表达实体、字段、主外键及基数，两者不能互相替代。
